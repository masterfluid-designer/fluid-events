...
export class AuthController {
   ...
   * (2026-08-23).
   *
   * `@Public` par nécessité : celui qui l'appelle ne peut pas se connecter,
   * c'est tout le problème. La réponse est TOUJOURS la même, qu'on connaisse
  ...
  async forgotPassword(@Body() dto: ForgotPasswordDto) {
    return this.passwordReset.demander(dto.email);
  }

  /**
   * POST /api/auth/reset-password — pose le nouveau mot de passe.
   *
  ...
  async resetPassword(@Body() dto: ResetPasswordDto) {
    return this.passwordReset.reinitialiser(dto.token, dto.password);
  }
