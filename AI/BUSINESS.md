...
- ~~Un seul compte/config Kkiapay global (`PaymentProviderConfig.provider` `@unique`) — pas de clés par Manager en V1.~~ **Dépassé (décision produit 2026-07-13, Admin de la plateforme)** : la config de paiement est désormais **par événement** (`PaymentProviderConfig.eventId`, `@@unique([eventId, provider])`). L'Admin (`SUPER_ADMIN`) configure/active le fournisseur pour chaque événement individuellement ; le Manager ne voit qu'un statut (actif/inactif) et une alerte "contactez l'administrateur" si rien n'est configuré — il ne choisit jamais lui-même le fournisseur ni ne voit les identifiants. 1 Manager = 1 Event en V1 (§3) reste vrai, donc c'est de facto une config par Manager — assumé explicitement par le produit, pas un contournement de la contrainte V1. Les modalités de reversement aux organisateurs restent un point ouvert (§12).
...
- **Remboursements** : politique de remboursement client (délai, conditions) en cas d'annulation d'un événement par un Manager — non tranchée. Ce qui EST tranché (2026-07-13) : annuler un événement est une transition de statut (`PATCH /api/events/mine { status: 'CANCELLED' }`), pas une suppression — les commandes/billets déjà vendus restent en base intacts, aucun remboursement n'est déclenché automatiquement. Un événement `CANCELLED` disparaît de la page publique et bloque tout nouveau paiement (`EventsService.getPublicEventBySlug`, `PaymentsService` vérifient déjà `status === 'PUBLISHED'`) et bloque le scanner à l'entrée (`decideScan` renvoie `EXPIRED` si `event.status !== 'PUBLISHED'`) — sans qu'aucun code n'ait eu besoin de changer pour ça. Transition réversible : repasser en `PUBLISHED` republie l'événement.
- **Statuts de publication d'un événement** : cycle de vie complet toujours pas entièrement tranché (brouillon → publié → clôturé → archivé ?) — seule la transition publié ↔ annulé a une UI (dashboard Manager) et un comportement défini à ce jour. Pas de state-machine imposée côté backend (`UpdateEventDto` accepte toute valeur connue de l'enum).
