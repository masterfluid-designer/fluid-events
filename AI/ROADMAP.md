...
| Notifications | ✅ `PhoneService` (validation E.164) + `EmailService` (nodemailer/SMTP) + `WhatsappService` (Meta Cloud API) — notifications "billets prêts" après génération PDF (2026-07-14). WhatsApp nécessite un template approuvé dans Meta Business Manager avant tout envoi réel (voir Phase 5 §5) |
...
- [x] **Notifications WhatsApp** (2026-07-14) — `WhatsappService` (`apps/api/src/notifications/whatsapp.service.ts`) appelle directement l'API Meta Cloud (`POST https://graph.facebook.com/{version}/{phoneNumberId}/messages`, `type: "template"`) — **Meta Cloud API directement, pas Twilio** (le bloc `.env` "WHATSAPP (Twilio)" était une exploration jamais branchée, remplacé par les vraies variables Meta ; dépendance `twilio` retirée de `package.json`, jamais importée nulle part). Déclenché depuis `PdfProcessor.maybeSendTicketNotifications` (renommé depuis `maybeSendTicketEmail`), en plus de l'email, seulement si `PhoneService.normalizeForWhatsapp(client.phone)` renvoie un numéro valide — sinon email seul, jamais d'erreur.
